...
type (
	ISysLogRepo interface {
		...
		UpsertOne(ctx context.Context, data *fkratos_sys_model.SysLog) error
		// UpsertOneByTx Upsert一条数据(事务)
		UpsertOneByTx(ctx context.Context, tx *fkratos_sys_dao.Query, data *fkratos_sys_model.SysLog) error
		// CreateBatch 批量创建数据
		...
		UpdateOneWithZero(ctx context.Context, data *fkratos_sys_model.SysLog) error
		// UpdateOneWithZero 更新一条数据,包含零值(事务)
		UpdateOneWithZeroByTx(ctx context.Context, tx *fkratos_sys_dao.Query, data *fkratos_sys_model.SysLog) error
...
// UpsertOne Upsert一条数据
func (s *SysLogRepo) UpsertOne(ctx context.Context, data *fkratos_sys_model.SysLog) error {
	dao := fkratos_sys_dao.Use(s.db).SysLog
...
// UpsertOneByTx Upsert一条数据(事务)
func (s *SysLogRepo) UpsertOneByTx(ctx context.Context, tx *fkratos_sys_dao.Query, data *fkratos_sys_model.SysLog) error {
	dao := tx.SysLog
	err := dao.WithContext(ctx).Save(data)
	if err != nil {
		return err
...
// UpdateOneWithZero 更新一条数据,包含零值
func (s *SysLogRepo) UpdateOneWithZero(ctx context.Context, data *fkratos_sys_model.SysLog) error {
	dao := fkratos_sys_dao.Use(s.db).SysLog
...
func (s *SysLogRepo) UpdateOneWithZeroByTx(ctx context.Context, tx *fkratos_sys_dao.Query, data *fkratos_sys_model.SysLog) error {
	dao := tx.SysLog
	_, err := dao.WithContext(ctx).Where(dao.ID.Eq(data.ID)).Select(dao.ALL.WithTable("")).Updates(data)
	if err != nil {
		return err
...
func (s *SysLogRepo) FindMultiByPaginator(ctx context.Context, paginatorReq *orm.PaginatorReq) ([]*fkratos_sys_model.SysLog, *orm.PaginatorReply, error) {
	...
	var total int64
	whereExpressions, orderExpressions, err := paginatorReq.ConvertToGormExpression(fkratos_sys_model.SysLog{})
	if err != nil {
		return nil, nil, err
	}
	err = s.db.WithContext(ctx).Model(&fkratos_sys_model.SysLog{}).Select([]string{"*"}).Clauses(whereExpressions...).Count(&total).Error
	if err != nil {
	...
	err = s.db.WithContext(ctx).Model(&fkratos_sys_model.SysLog{}).Limit(paginatorReply.Limit).Offset(paginatorReply.Offset).Clauses(whereExpressions...).Clauses(orderExpressions...).Find(&result).Error
	if err != nil {
		return result, nil, err
	}
