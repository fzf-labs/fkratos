...
// CreateBatch 批量创建数据
func (r *SysAdminRepo) CreateBatch(ctx context.Context, data []*fkratos_sys_model.SysAdmin, batchSize int) error {
	dao := fkratos_sys_dao.Use(r.db).SysAdmin
...
// FindOneByUsername 根据username查询一条数据
func (r *SysAdminRepo) FindOneByUsername(ctx context.Context, username string) (*fkratos_sys_model.SysAdmin, error) {
	dao := fkratos_sys_dao.Use(r.db).SysAdmin
	result, err := dao.WithContext(ctx).Where(dao.Username.Eq(username)).First()
	if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
		return nil, err
...
// FindMultiByUsernames 根据usernames查询多条数据
func (r *SysAdminRepo) FindMultiByUsernames(ctx context.Context, usernames []string) ([]*fkratos_sys_model.SysAdmin, error) {
	dao := fkratos_sys_dao.Use(r.db).SysAdmin
