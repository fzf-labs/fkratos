...
func newSysJob(db *gorm.DB, opts ...gen.DOOption) sysJob {
	...
	_sysJob.sysJobDo.UseModel(&rpc_sys_model.SysJob{})
...
func (s sysJobDo) Create(values ...*rpc_sys_model.SysJob) error {
	if len(values) == 0 {
		return nil
	}
...
func (s sysJobDo) CreateInBatches(values []*rpc_sys_model.SysJob, batchSize int) error {
	return s.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (s sysJobDo) Save(values ...*rpc_sys_model.SysJob) error {
	if len(values) == 0 {
		return nil
	}
...
func (s sysJobDo) First() (*rpc_sys_model.SysJob, error) {
	if result, err := s.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*rpc_sys_model.SysJob), nil
	}
}

func (s sysJobDo) Take() (*rpc_sys_model.SysJob, error) {
...
func (s sysJobDo) Last() (*rpc_sys_model.SysJob, error) {
...
func (s sysJobDo) Find() ([]*rpc_sys_model.SysJob, error) {
	result, err := s.DO.Find()
	return result.([]*rpc_sys_model.SysJob), err
}

func (s sysJobDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*rpc_sys_model.SysJob, err error) {
	buf := make([]*rpc_sys_model.SysJob, 0, batchSize)
...
func (s sysJobDo) FindInBatches(result *[]*rpc_sys_model.SysJob, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (s sysJobDo) FirstOrInit() (*rpc_sys_model.SysJob, error) {
...
func (s sysJobDo) FirstOrCreate() (*rpc_sys_model.SysJob, error) {
...
func (s sysJobDo) FindByPage(offset int, limit int) (result []*rpc_sys_model.SysJob, count int64, err error) {
...
func (s sysJobDo) Delete(models ...*rpc_sys_model.SysJob) (result gen.ResultInfo, err error) {
