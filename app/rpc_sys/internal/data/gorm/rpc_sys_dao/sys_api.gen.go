...
func newSysAPI(db *gorm.DB, opts ...gen.DOOption) sysAPI {
	...
	_sysAPI.sysAPIDo.UseModel(&rpc_sys_model.SysAPI{})
...
func (s sysAPIDo) Create(values ...*rpc_sys_model.SysAPI) error {
	if len(values) == 0 {
		return nil
	}
...
func (s sysAPIDo) CreateInBatches(values []*rpc_sys_model.SysAPI, batchSize int) error {
	return s.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (s sysAPIDo) Save(values ...*rpc_sys_model.SysAPI) error {
	if len(values) == 0 {
		return nil
	}
...
func (s sysAPIDo) First() (*rpc_sys_model.SysAPI, error) {
	if result, err := s.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*rpc_sys_model.SysAPI), nil
	}
}

func (s sysAPIDo) Take() (*rpc_sys_model.SysAPI, error) {
...
func (s sysAPIDo) Last() (*rpc_sys_model.SysAPI, error) {
...
func (s sysAPIDo) Find() ([]*rpc_sys_model.SysAPI, error) {
	result, err := s.DO.Find()
	return result.([]*rpc_sys_model.SysAPI), err
}

func (s sysAPIDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*rpc_sys_model.SysAPI, err error) {
	buf := make([]*rpc_sys_model.SysAPI, 0, batchSize)
...
func (s sysAPIDo) FindInBatches(result *[]*rpc_sys_model.SysAPI, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (s sysAPIDo) FirstOrInit() (*rpc_sys_model.SysAPI, error) {
...
func (s sysAPIDo) FirstOrCreate() (*rpc_sys_model.SysAPI, error) {
...
func (s sysAPIDo) FindByPage(offset int, limit int) (result []*rpc_sys_model.SysAPI, count int64, err error) {
...
func (s sysAPIDo) Delete(models ...*rpc_sys_model.SysAPI) (result gen.ResultInfo, err error) {
